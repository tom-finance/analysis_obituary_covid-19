...
data_ma_st <- meltdf %>%
  ...
  mutate(roll_mean = rollmean(value, 7, na.pad = T)) %>% # calculate moving average
...
ggplot(data_ma_st %>%
  ...
  geom_line(aes(y = roll_mean), color="steelblue") +
  theme_minimal() +
  geom_hline(yintercept=0, linetype="dashed", color = "orangered", size = 1)
...
data_ma <- data %>%
  ...
  mutate(roll_mean = rollmean(workplaces_percent_change_from_baseline, 30, na.pad = T)) %>%
